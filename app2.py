...
def update_details_table(clickData, department, division, costcenter, variance_range, group_col):
    ...
    for col in ordered_cols:
        ...
        if pd.api.types.is_numeric_dtype(table_df[col]):
            ...
            # Színezés a szám oszlopoknál
            if col == "Budgeted_Amount":
                coldef["cellStyle"] = {"backgroundColor": "#e3f2fd", "fontWeight": "bold"}
            elif col == "Actual_Amount":
                coldef["cellStyle"] = {"backgroundColor": "#e8f5e9", "fontWeight": "bold"}
            elif col == "Variance":
                coldef["cellStyle"] = {"backgroundColor": "#fff3e0", "fontWeight": "bold"}
        else:
